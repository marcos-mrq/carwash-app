<?php 
  ...
  if(isset($_POST['btn-rel'])){

    header("Location: relatorio.php?inicio=".$_POST['input_data_ini']."&fim=".$_POST['input_data_fim']."&nome=".$_POST['text-search']);
    // echo "Arroz doce";
  ...
  }elseif(isset($_POST['btn-relatorio'])){

    header("Location: relatorio_users.php?inicio=".$_POST['input_data_ini']."&fim=".$_POST['input_data_fim']."&nome=".$_POST['text-search']);

  }else{
    $_SESSION['msg'] = "<script>Swal.fire('Erro de permissão','Não pode acessar à esta página!','error');</script>";
              ...
              <div class="card card-primary card-outline">
                <div class="card-header">
                  ...
                  <div class="input-group input-group-sm">  
                      <input type="text" class="form-control" placeholder="pesquise aqui a marca do veículo" name="text-search">
                      ...
                      <tbody>
                        <tr>
                          ...
                          <?php 
                            ...
                            $click = $_POST['btn-search'];
                            $texto = (isset($_POST["text-search"])) ? $_POST["text-search"] : empty($texto);
                            $data_ini = (isset($_POST["input_data_ini"])) ? $_POST["input_data_ini"] : empty($data_ini);
                            $data_fim = (isset($_POST["input_data_fim"])) ? $_POST["input_data_fim"] : empty($data_fim);

                            if (isset($click)) {
                                if(($texto != "") && (empty($data_ini)) && (empty($data_fim))){
                                  $sql = "SELECT * from veiculo where Marca_veiculo like '%$texto%' order by Id desc";
                                  $q = $Conn->query("SELECT Nome from usuarios JOIN veiculo ON usuarios.Id = veiculo.id_user where veiculo.Marca_veiculo like '%$texto%' order by veiculo.Id desc"); 
                                  $sq = $Conn->query($sql);
                                  $num_lines = mysqli_num_rows($sq);
                                  if ($num_lines > 0) {
                                    while ($linhas = $sq->fetch_assoc()) {
                                      $user = $q->fetch_assoc();
                                      $id = $linhas['Id']; 
                                      echo "<td>".$linhas['Marca_veiculo']."</td>";
                                  ...
                                  }else{ echo "<div class='alert alert-danger' role='alert'>Nenhum resultado encontrado!</div>";}
                                  
                                }
                                  
                                elseif( ($texto != "") && !(empty($data_ini)) && !(empty($data_fim)) ){
                                    $sql = "SELECT * from veiculo where Marca_veiculo like '%$texto%' AND Data_registro >= '$data_ini%' AND Data_registro <= '$data_fim%' order by Id desc";
                                    $q = $Conn->query("SELECT Nome from usuarios JOIN veiculo ON usuarios.Id = veiculo.id_user where veiculo.Marca_veiculo like '%$texto%' AND veiculo.Data_registro >= '$data_ini%' AND veiculo.Data_registro <= '$data_fim%' order by veiculo.Id desc");
                                    $sq = $Conn->query($sql);
                                    $num_lines = mysqli_num_rows($sq);
                                    if ($num_lines > 0) {
                                      while ($linhas = $sq->fetch_assoc()) {
                                ...
                                elseif( (empty($texto)) && !(empty($data_ini)) && !(empty($data_fim)) ){
                                  $sql = "SELECT * from veiculo where Data_registro BETWEEN '$data_ini%' AND '$data_fim%' order by Id desc";
                                  $q = $Conn->query("SELECT Nome from usuarios JOIN veiculo ON usuarios.Id = veiculo.id_user where veiculo.Data_registro BETWEEN '$data_ini%' AND '$data_fim%' order by veiculo.Id desc"); 
                                  $sq = $Conn->query($sql);
                                  $num_lines = mysqli_num_rows($sq);
                                    if ($num_lines > 0) {
                                      while ($linhas = $sq->fetch_assoc()) {
                                  ...
                                  elseif( (empty($texto)) && !(empty($data_ini)) && (empty($data_fim)) ){
                                    $sql = "SELECT * from veiculo where Data_registro LIKE '$data_ini%' order by Id desc";
                                    $q = $Conn->query("SELECT Nome from usuarios JOIN veiculo ON usuarios.Id = veiculo.id_user where veiculo.Data_registro like '%$data_ini%' order by veiculo.Id desc");
                                    $sq = $Conn->query($sql);
                                    $num_lines = mysqli_num_rows($sq);
                                    if ($num_lines > 0) {
                                      while ($linhas = $sq->fetch_assoc()) {
                                ...
                                elseif( ($texto != "") && !(empty($data_ini)) && (empty($data_fim)) ){
                                  $sql = "SELECT * from veiculo where Marca_veiculo like '%$texto%' AND Data_registro LIKE '%$data_ini%' order by Id desc";
                                  $q = $Conn->query("SELECT Nome from usuarios JOIN veiculo ON usuarios.Id = veiculo.id_user where veiculo.Marca_veiculo like '%$texto%'AND veiculo.Data_registro LIKE '%$data_ini%' order by veiculo.Id desc");
                                  $sq = $Conn->query($sql);
                                  $num_lines = mysqli_num_rows($sq);
                                  if ($num_lines > 0) {
                                    while ($linhas = $sq->fetch_assoc()) {
